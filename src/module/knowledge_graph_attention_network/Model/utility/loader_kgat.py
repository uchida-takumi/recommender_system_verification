...
"""　デバッグ用
import numpy as np
from src.module.knowledge_graph_attention_network.Model.utility.load_data import Data
from time import time
import scipy.sparse as sp
import random as rd
import collections
...
class KGAT_loader(Data):
    ...
    def _get_relational_adj_list(self):
        ...
        def _np_mat2sp_adj(np_mat, row_pre, col_pre):
            """
            np_mat : interactions data

            np_mat = self.train_data
            row_pre = 0 
            col_pre = self.n_users   
            
            n_user + n_item + n_attribute の隣接行列を生成する
            """
            # n_entities はアイテム数+アイテム属性情報のノード数
            ...
            # a_adj は n_all * n_all の行列で、b_adjはその逆行列
            # adj は隣接行列を表している。
        ...
        # R は user-item の interaction 隣接行列。
        # しかし、user, item, attribute を同じインデックスで管理するため、行列サイズはentity*entity
        R, R_inv = _np_mat2sp_adj(self.train_data, row_pre=0, col_pre=self.n_users)
        # adj_mat_list は隣接行列のリスト
        adj_mat_list.append(R)
        # adj_r_list には　adj_mat_list のエッジタイプを保存する。user-item interaction は index 0 と定義される。
        adj_r_list.append(0)
        ...
        # item-attribute の隣接行列を生成する。これは定義されたrelationタイプごとに生成される。
